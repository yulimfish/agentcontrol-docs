...
features:
  - icon: 🎮
    title: Fabric 客户端模组
    details: 在本地 Minecraft 客户端运行，暴露状态读取和动作执行 HTTP 端点。支持 1.21.1，零服务端依赖。
  - icon: 🤖
    title: MCP 服务集成
    details: 一句话让 AI 助手安装配置。提供 mod_move_player、mod_look、mod_attack 等工具，直接调用本地模组。
  - icon: 🔒
    title: 安全边界清晰
  ...
  - icon: 🛠️
    title: 可选 Mod Menu
    details: 兼容 Mod Menu 配置界面。支持中英文本地化，鼠标释放行为可在游戏内设置。
  - icon: 🌐
    title: 开源 & 可扩展
    details: 基于 MIT 许可发布。代码清晰模块化，可扩展更多状态读取和动作类型。欢迎提交 PR 和 Issue。
---
